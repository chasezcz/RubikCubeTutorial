    ...
    def solve(colors, startIdx=0, endIdx=-1):
        ...
        for face in [0, 3, 5, 1, 2, 4]:
            for j in range(2, -1, -1):
                for i in range(0, 3):
                    kociembaOrder = np.append(
                        kociembaOrder, np.ravel_multi_index((face, i, j), (6, N, N)))

        colors_kociemba_ordered = colors[kociembaOrder]
        colors_to_kociemba_input = {
    ...
    def solve(states, startIdx=0, endIdx=-1, combineOutputs=False, solveInputs=None):
        ...
        if solveInputs is None:
            ...
            for state in states:
                ...
                solveInput = "".join(
                    [x[0] + "1" if x[1] == -1 else x[0] + "3" for x in kocSoln_backwards])
        ...
        os.system('cd ./solvers/cube3/rokicki/ && ./nxopt33 -t 40 - < %s > %s' %
...
class Optimal_2:
    ...
    def solve(colors):
        ...
        kocSoln_backwards = kocSoln[::-1]
        solveInput = " ".join(
            [x[0] if x[1] == -1 else x[0] + "'" for x in kocSoln_backwards])
...
class Korf:
    ...
    def solve(colors):
        ...
        result = subprocess.check_output(
            ['./twist.out', twistInput], cwd='./solvers/cube3/korf_2/', stderr=subprocess.STDOUT)
        korf_rep = result.split('\n')[-3]

        # Solve cube
        #print("Korf Rep: %s" % (korf_rep))
        result = subprocess.check_output(
            ['./a.out', korf_rep], cwd='./solvers/cube3/korf_2/', stderr=subprocess.STDOUT)
        moves_korf = [x for x in result.split(
            "\n")[-3].split(" ") if '(' not in x and ')' not in x]
        #print("Korf Moves: %s" % (moves_korf))
...
class Korf_2:
    ...
    def solve(colors):
        ...
        for j in range(2, -1, -1):
            for i in range(0, 3):
                if i == 1 and j == 1:
                    continue
                korfOrder = np.append(
                    korfOrder, np.ravel_multi_index((face, i, j), (6, N, N)))
        ...
        # Get solution
        result = subprocess.check_output(
            ['./solver', korfRep], cwd='./solvers/cube3/korf/', stderr=subprocess.STDOUT)
        moves_korf = result.split('\n')[-2].split(" ")

        # Add moves
        soln_to_move = {'T': 'U', 'L': 'L',
                        'R': 'R', 'D': 'D', 'B': 'B', 'F': 'F'}
        moves = []
        for move_korf in moves_korf:
            if len(move_korf) == 0:
